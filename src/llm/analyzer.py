# -*- coding: utf-8 -*-
"""GeminiAnalyzer — high-level stock analysis facade.

This module hosts the GeminiAnalyzer class and its private helpers (deep
analysis orchestrator, multi-agent specialists, batch analysis). It builds
on src/llm/client.py (provider layer) and src/llm/prompts.py / parsing.py
(shared modules) to provide stock-specific analysis behavior.

Migrated from src/analyzer.py in P0-5 Phase 9. The old src/analyzer.py
file becomes a thin re-export shim for backward compat with the 17+
import sites.
"""
